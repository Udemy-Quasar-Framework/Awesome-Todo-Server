...
module.exports.auth = async (req, res, next) => {
    ...
    if (!token) {
        ...
        res.statusCode = 401;
        res.send(new Res(false, 'Token not provided', [
    ...
    try {
        ...
        if (lastAuthToken !== token) {
            debug('Invalid token. The token is JWT valid but is not the last token provided.');
            const errorMsg = 'Invalid Token. The token is not valid for the current session.';
            res.statusCode = 401;
            res.send(new Res(false, errorMsg, [
